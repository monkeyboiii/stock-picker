...
def build_stmt_postgresql_lateral(trade_day: date) -> Select:
    ...
    stmt = (
        select(
            ...
            (100.0 * (static_filtering_cte.c.close / prev_subq.c.close - 1)).label(
                "gain"
            ),
        ...
        .where(
            # T1
            (100.0 * (static_filtering_cte.c.close / prev_subq.c.close - 1)).between(
                3, 5
            ),
            # T5
...
def build_stmt_postgresql_mv(mv_stock_daily: Table, trade_day: date) -> Select:
    ...
    stmt = (
        select(
            ...
            (100 * (sd.c.volume.cast(Double) / prev.c.volume.cast(Double) - 1)).label(
                "volume_gain"
            ),
        )
...
def build_stmt_postgresql(engine: Engine, trade_day: date) -> Select:
    if check_mv_exists(engine, MV_STOCK_DAILY):
        mv_stock_daily = Table(
            MV_STOCK_DAILY, MetadataBase.metadata, autoload_with=engine
        )
...
def filter_desired(
    ...
    if engine.dialect.name == "postgresql":
        ...
        logger.debug(
            filter_stmt.compile(engine, compile_kwargs={"literal_binds": True})
        )
    else:
    ...
    with Session(engine) as session:
        ...
        for result in results:
            fd = FeedDaily(
                filter_id=get_filter_id(StockFilter.TAIL_SCRAPER), **result._mapping
            )
            output.append(fd)
        ...
        if not dryrun:
            if engine.dialect.name == "postgresql":
                ...
                insert_stmt = insert_stmt.on_conflict_do_update(
                    index_elements=["code", "trade_day", "filter_id"], set_=update_dict
                )
