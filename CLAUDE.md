...
Automated unit tests run on every pull request to `main`:
...
**GitHub Free Tier:**
- ✅ 2,000 CI/CD minutes/month (free)
- ✅ This workflow easily fits within limits
- ✅ No cost for public repositories

**Status Checks:**
Configure branch protection (Settings → Branches → main):
- ✅ Require status checks to pass before merging
- ✅ Enable "Allow force pushes" if needed
...
# Linting (auto-runs on commit, or manually)
ruff check .           # Check for issues
ruff check . --fix     # Fix imports, whitespace
...
This project uses **minimal, non-invasive linting**:

**Philosophy:**
- ✅ Organize imports, remove trailing whitespace, catch syntax errors
- ❌ No line length enforcement, no indentation changes

**Ruff Configuration** (`pyproject.toml`):
- Python 3.13+ target
- Enabled checks: Pyflakes (F), isort (I), trailing whitespace (W291-293), pyupgrade (UP)
- Format disabled - respects your editor's indentation
...
git commit -m "message"  # Auto-fixes imports, whitespace
...
```bash
ruff check .           # Check for issues
ruff check . --fix     # Auto-fix imports, whitespace
pre-commit run --all-files  # Run all hooks manually
```

**See:** `docs/LINTING.md` for detailed guide.
